...
import { fetchAgents, clearAgentMemory, clearAllAgentsMemory, fetchGatewayStats } from '../api/client';
...
function ConfirmButton({ label, onConfirm, danger = true }) {
  ...
  if (state === 'working') return <span className="text-xs text-navy-500">Clearing...</span>;
  if (state === 'done')    return <span className="text-xs text-emerald-700">Cleared</span>;
  if (state === 'error')   return <span className="text-xs text-rose-700">{errorMsg}</span>;
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-navy-500">
        Memory cleared here takes effect immediately — agents will rebuild it on their next run cycle.
      </p>
          ...
          <div className="space-y-2">
            {agents.map((agent) => (
              <div key={agent.id} className="flex items-center justify-between rounded-lg border border-navy-200 p-3">
                ...
                <ConfirmButton label="Clear memory" onConfirm={() => clearAgentMemory(agent.id)} />
              </div>
            ))}
          </div>
        )}
