...
function ConfirmButton({ label, description, onConfirm, danger = true }) {
  ...
  if (state === 'done') {
    return <span className="text-xs text-emerald-400">Cleared</span>;
  }

  if (state === 'error') {
    return <span className="text-xs text-rose-400">{errorMsg}</span>;
  }
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-semibold transition-colors ${
        danger
          ? 'border-rose-800/60 text-rose-400 hover:border-rose-600 hover:bg-rose-600/10'
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center justify-between rounded-lg border border-slate-800 p-4">
          <div>
            <p className="text-sm font-medium text-slate-200">Clear all agent memory</p>
      ...
      >
        ...
        {agents && (
          <div className="space-y-2">
            {agents.map((agent) => (
              <div key={agent.id} className="flex items-center justify-between rounded-lg border border-slate-800 p-3">
                <div>
                  <p className="text-sm font-medium text-slate-200">{agent.hostname}</p>
                  <p className="mt-0.5 text-xs text-slate-500">{agent.role} &middot; {agent.os}</p>
                </div>
                <ConfirmButton
                  label="Clear memory"
